...
* **Annual Bank Wages (Descending Circle Chart):** A wrapping, horizontal sequence of visual circles of descending sizes representing interest payments grouped by calendar year. It features area-based (square-root) scaling, custom glassmorphic styling, responsive wrapping, and a run-rate back-fill algorithm for mortgages starting mid-year (extrapolating the first year's annual run-rate). It includes a premium segmented switch toggle allowing users to transition the chart between **Bank Wages** (annual interest payments), **Rent Equivalent** (interest averaged monthly), and **Rent + Tax & Insurance** (combined monthly carrying costs of rent equivalent + property tax/12 + home insurance/12). The Rent + Tax & Insurance toggle conditionally displays only when the main Taxes & Insurance (PITI) option is activated, and hovering over its circles reveals a stacked vertical breakdown of its components (Rent, Tax, and Insurance).
