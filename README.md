...
* **Annual Bank Wages (Descending Circle Chart):** A wrapping, horizontal sequence of visual circles of descending sizes representing interest payments grouped by calendar year. It features area-based (square-root) scaling, custom glassmorphic styling, responsive wrapping, and a run-rate back-fill algorithm for mortgages starting mid-year (extrapolating the first year's annual run-rate).
